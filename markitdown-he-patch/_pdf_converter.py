...
# Hebrew and Arabic Unicode letter ranges (RTL scripts)
_RTL_CHAR_PATTERN = re.compile(r"[֐-׿؀-ۿ]")
_LTR_CHAR_PATTERN = re.compile(r"[A-Za-z]")


def _is_rtl_dominant(text: str) -> bool:
    """
    Check whether RTL (Hebrew/Arabic) letters outnumber LTR (Latin) letters
    in the given text.

    pdfminer.high_level.extract_text() can mangle RTL text layout (reversed
    words, fragmented lines) for some PDFs. When text is RTL-dominant, the
    whole-document pdfminer fallback should be skipped in favor of
    pdfplumber's per-page extraction, which doesn't have this issue.
    """
    rtl_count = len(_RTL_CHAR_PATTERN.findall(text))
    ltr_count = len(_LTR_CHAR_PATTERN.findall(text))
    ...
    def convert(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # unless the text is RTL-dominant (Hebrew/Arabic), since
            # pdfminer's layout analysis mangles RTL line/word order
            # while pdfplumber's per-page extraction (already collected
            # above) does not.
            plain_text = "\n\n".join(markdown_chunks).strip()
            if form_page_count == 0 and not _is_rtl_dominant(plain_text):
                pdf_bytes.seek(0)
